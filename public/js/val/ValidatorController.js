...
app.controller('validatorController', function ($scope, $http, $window) {
  ...
  this.parseMarkup = function(thing) {
    try {
      ...
      console.log('no es json, pruebe con yaml');
  ...
  this.validateDocument = function () {
    ...
    try {
      ...
      if (documentValidator.errors && documentValidator.errors.length) {
        ...
        this.documentMessage = "El Documento esta conforme a el JSON SCHEMA ingresado.";
      }
    } catch (e) {
      // Error parsing as JSON
      self.documentErrors = [{message: "El documento es un JSON no valido." }];
    }
  ...
  this.validateSchema = function () {
    ...
    try {
      ...
      if (schemaValidator.errors && schemaValidator.errors.length) {
        ...
        this.schemaMessage = "El Schema es un 'JSON schema' valido.";
      }
    } catch (e) {
      // Error parsing as JSON
      self.schemaErrors = [{ message: "El Schema is 'JSON schema' no válido." }];
    }
